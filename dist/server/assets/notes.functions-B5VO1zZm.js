...
function generateLocalNotesAnalysis(subject) {
  const syllabusGapsMap = {
    DBMS: {
      coverageScore: 82,
      ...
      recommendations: ["Include clear worked examples for 3NF and BCNF normalization.", "Add short definitions for transaction isolation levels (Dirty Read, Phantom Read).", "Add a visual diagram of B+ tree node splitting."],
      revisionSheet: {
        summary: "Database management notes are solid on SQL but have severe gaps in Normalization and Indexing.",
        formulas: ["Closure of attribute set: X+ = X, then iteratively add A if Y -> A and Y is subset of X+", "ACID properties: Atomicity, Consistency, Isolation, Durability"],
        tips: ["In viva, examiners frequently ask why B+ trees are preferred over binary trees (answer: disk I/O reduction).", "Ensure you can decompose a relation step-by-step into 3NF."]
      },
      readinessScore: 81,
      healthStatus: "Yellow"
    }
  ...
  const defaultAnalysis = {
    ...
    conceptDepth: [{
      ...
      topic: "Frameworks & Standard Layers",
      percent: 60
    }, {
    ...
    recommendations: ["Review notes against standard syllabus textbook modules.", "Add real-world implementation case studies."],
    revisionSheet: {
...
const uploadAndAnalyzeNotes_createServerFn_handler = createServerRpc({
  id: "7c98ed4cc526909e256b29e8fdb5dc5c9c984117e900cd9dcb250e27e27bd943",
  name: "uploadAndAnalyzeNotes",
  filename: "src/lib/notes.functions.ts"
}, (opts) => uploadAndAnalyzeNotes.__executeServer(opts));
const uploadAndAnalyzeNotes = createServerFn({
  method: "POST"
}).middleware([requireSupabaseAuth]).inputValidator((input) => NotesInputSchema.parse(input)).handler(uploadAndAnalyzeNotes_createServerFn_handler, async ({
  data,
  context
}) => {
  const {
  ...
  const truncatedText = notesText.slice(0, 2e4);
...
Return a valid JSON object (no markdown blocks) with this schema: {"coverageScore":0,"missingTopics":[],"conceptDepth":[],"weakAreas":{"highRisk":[],"mediumRisk":[],"lowRisk":[]},"recommendations":[],"revisionSheet":{"summary":"","formulas":[],"tips":[]},"readinessScore":0,"healthStatus":""}`;
  ...
  const {
    data: row,
    error
  } = await supabaseServer.from("notes_analyses").insert([{
    user_id: userId,
...
const listNotesAnalyses_createServerFn_handler = createServerRpc({
  id: "c5e175fa05465f5d215fbdc003bcc4610aaba60f6083f1073967ff7794aee63d",
  name: "listNotesAnalyses",
  filename: "src/lib/notes.functions.ts"
}, (opts) => listNotesAnalyses.__executeServer(opts));
const listNotesAnalyses = createServerFn({
  method: "GET"
}).middleware([requireSupabaseAuth]).handler(listNotesAnalyses_createServerFn_handler, async ({
  context
}) => {
  const {
    userId
  ...
  const {
    data,
    error
  } = await supabaseServer.from("notes_analyses").select("id, file_name, num_pages, subject, status, result, created_at").eq("user_id", userId).order("created_at", {
    ascending: false
  });
  if (error) {
...
const deleteNotesAnalysis_createServerFn_handler = createServerRpc({
  id: "081f8da933e521b0d47cfcbef4ad0231b0789f257086b1918daaf729bafe4d18",
  name: "deleteNotesAnalysis",
  filename: "src/lib/notes.functions.ts"
}, (opts) => deleteNotesAnalysis.__executeServer(opts));
const deleteNotesAnalysis = createServerFn({
  method: "POST"
}).middleware([requireSupabaseAuth]).inputValidator((input) => z.object({
  id: z.string()
}).parse(input)).handler(deleteNotesAnalysis_createServerFn_handler, async ({
  data,
  context
}) => {
  const {
  ...
  const {
    error
  } = await supabaseServer.from("notes_analyses").delete().eq("id", data.id).eq("user_id", userId);
  if (error) {
...
export {
  deleteNotesAnalysis_createServerFn_handler,
  listNotesAnalyses_createServerFn_handler,
  uploadAndAnalyzeNotes_createServerFn_handler
};
