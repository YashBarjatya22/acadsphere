...
function analyzeResumeHeuristics(resumeText, jobDescription) {
  const rWords = new Set(resumeText.toLowerCase().match(/\b\w+\b/g) || []);
  const jdWords = new Set(jobDescription.toLowerCase().match(/\b\w+\b/g) || []);
  const skillsDb = ["python", "javascript", "typescript", "react", "vue", "angular", "node", "fastapi", "django", "flask", "sql", "nosql", "mongodb", "postgresql", "mysql", "sqlite", "docker", "kubernetes", "aws", "gcp", "azure", "git", "github", "ci/cd", "agile", "scrum", "machine learning", "data science", "deep learning", "nlp", "html", "css", "tailwind", "bootstrap", "next.js", "vite", "java", "c++", "c#", "go", "rust", "php", "communication", "leadership", "problem solving", "teamwork", "analytical", "project management"];
...
const analyzeResume = createServerFn({
  method: "POST"
}).middleware([requireSupabaseAuth]).inputValidator((input) => ResumeInputSchema.parse(input)).handler(analyzeResume_createServerFn_handler, async ({
  data
}) => {
  ...
  try {
    let model2;
    if (customKey) {
      model2 = getAiModelWithCustomKey(customKey, data.provider);
    } else {
      model2 = getAiModel();
    }
    const prompt = `
